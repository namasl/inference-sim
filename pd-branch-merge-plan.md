# PD Branch Merge Plan: Prefill-Decode Disaggregation Feature

## Overview

This document outlines the strategy for merging the `pd` branch into `main` through a series of small, focused pull requests. The PD branch implements prefill-decode disaggregation simulation capability, allowing BLIS to model architectures where prefill (prompt processing) and decode (token generation) run on separate instance pools.
