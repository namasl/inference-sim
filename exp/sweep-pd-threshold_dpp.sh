...
# H100 TP=4 cluster running meta-llama/llama-3.3-70b-instruct.
#
# For each workload (interactive-chat, code-generation), runs every threshold
# in THRESHOLDS and writes a per-workload CSV plus a stdout summary table
# sorted by E2E p99 (lower = better).
#
# Output: $OUTDIR (default /tmp/blis-pd-sweep)
#   <workload>.yaml             # rate-adjusted workload spec
#   <workload>-<thr>.json       # full metrics JSON
#   <workload>-<thr>.log        # blis stdout (includes PD metrics)
#   <workload>.csv              # one row per threshold
#
# Usage:
#   examples/sweep-pd-threshold.sh                  # default rates (50, 0.1)
#   CHAT_RATE=80 CODE_RATE=0.15 examples/sweep-pd-threshold.sh
#   OUTDIR=./results examples/sweep-pd-threshold.sh

set -euo pipefail

cd "$(dirname "$0")/.."

OUTDIR="${OUTDIR:-./blis-pd-sweep-thresh_dpp}"
...
# blis-cost-prop carries the cost-proportional decider patch (applied to the
# working tree but not yet committed; see sim/disaggregation.go). Build it
# unconditionally so it always reflects the current working tree.
go build -o blis-cost-prop main.go
...
run_sweep() {
  ...
  # Cost-proportional reference run — best algorithm per .nous/pd2/report.md.
  # budget=2×floor is the scale-consistent safe budget (RP-44, iters 14-16);
  # floor=1000 is the minimum safe floor at rate≥5 (RP-57/58, iter 20).
  # Parameters validated across seeds and rates 5-8 on 1P+3D H100 TP=4.
  local cp_metrics="$OUTDIR/${wl_name}-cost-prop.json"
  local cp_log="$OUTDIR/${wl_name}-cost-prop.log"
  printf "  cost-prop          ... "
  ./blis-cost-prop run \
    --model "$MODEL" \
    --workload-spec "$cfg" \
    ...
    --prefill-tp 4 --decode-tp 4 --hardware H100 \
    --pd-transfer-bandwidth 10.3 \
    --pd-decider cost-proportional --pd-cost-budget 2000 --pd-cost-floor 1000 \
    --post-hoc-detector composite \
    --metrics-path "$cp_metrics" > "$cp_log" 2>&1
  local cp_disagg
  cp_disagg=$(grep -oE 'Disaggregated Requests: [0-9]+' "$cp_log" | grep -oE '[0-9]+$' || echo 0)
  local cp_e2e cp_ttft
  cp_e2e=$(jq -r '.e2e_p99_ms // 0' "$cp_metrics")
  cp_ttft=$(jq -r '.ttft_p99_ms // 0' "$cp_metrics")
  printf "ttft_p99=%8.1fms  e2e_p99=%9.1fms  disagg=%s\n" "$cp_ttft" "$cp_e2e" "$cp_disagg"

  # blis-dpp reference runs: bernoulli (random f=0.5) and DPP (dual-price proportional).
  local dpp_deciders=(
    "bernoulli|--pd-decider=bernoulli --bernoulli-f=0.5"
    "dpp|--pd-decider=dpp --dpp-v=100 --dpp-eta=1.0 --dpp-ttft-slo-d=50"
  )
  for spec in "${dpp_deciders[@]}"; do
    local dpp_label="${spec%%|*}"
    local dpp_flags="${spec#*|}"
    local dpp_metrics="$OUTDIR/${wl_name}-${dpp_label}.json"
    local dpp_log="$OUTDIR/${wl_name}-${dpp_label}.log"
    printf "  %-18s ... " "$dpp_label"
    # shellcheck disable=SC2086
    ./blis-dpp run \
      --model "$MODEL" \
      --workload-spec "$cfg" \
      ...
      $dpp_flags \
      --post-hoc-detector composite \
      --metrics-path "$dpp_metrics" > "$dpp_log" 2>&1
    local dpp_disagg
    dpp_disagg=$(grep -oE 'Disaggregated Requests: [0-9]+' "$dpp_log" | grep -oE '[0-9]+$' || echo 0)
    local dpp_e2e dpp_ttft
    dpp_e2e=$(jq -r '.e2e_p99_ms // 0' "$dpp_metrics")
    dpp_ttft=$(jq -r '.ttft_p99_ms // 0' "$dpp_metrics")
    printf "ttft_p99=%8.1fms  e2e_p99=%9.1fms  disagg=%s\n" "$dpp_ttft" "$dpp_e2e" "$dpp_disagg"
  done
}
