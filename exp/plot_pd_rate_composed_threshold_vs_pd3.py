...
Reads blis-pd-sweep-rate_composed_threshold_vs_pd3/composed.csv and produces
...
DATA_DIR = "./blis-pd-sweep-rate_composed_threshold_vs_pd3"
CSV_PATH = f"{DATA_DIR}/composed.csv"

ARMS = ["agg-4xtp4", "pd-thresh16", "pd-pd3"]
...
def plot_pd3_vs_baseline_delta(data, chats, codes, col, title, lower_better, out):
    """Per-cell pd3 - threshold16 difference. Numbers carry the metric's
    natural sign; the colormap encodes who wins (blue = pd3 wins) so the
    visual story is consistent across metrics regardless of polarity."""
    g_pd3 = make_grid(data.get("pd-pd3", {}), chats, codes, col)
    g_t16 = make_grid(data.get("pd-thresh16", {}), chats, codes, col)
    delta = g_pd3 - g_t16
    if lower_better:
        cmap = "RdBu_r"  # blue at low end → negative delta (pd3 wins) is blue
        cbar_label = f"{title}: pd3 − threshold16 (negative = pd3 wins)"
    else:
        cmap = "RdBu"    # blue at high end → positive delta (pd3 wins) is blue
        cbar_label = f"{title}: pd3 − threshold16 (positive = pd3 wins)"
    ...
    ax.set_xlabel("chat_rate")
    ax.set_ylabel("code_rate")
    ax.set_title(f"pd3 vs threshold16 — {title}")
...
def main():
    ...
    for col, title, lower_better in METRICS:
        ...
        if "pd-pd3" in data and "pd-thresh16" in data:
            plot_pd3_vs_baseline_delta(data, chats, codes, col, title, lower_better,
                                       f"{DATA_DIR}/composed-{col}-pd3-vs-thresh16.png")
