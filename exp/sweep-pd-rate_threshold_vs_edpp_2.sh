...
#   pd-edpp      — 1P+3D TP=4, edpp decider (no extra options).
#
...
# seed) cell and writes a per-workload CSV plus a stdout summary table.
#
# Binaries:
#   blis      — standard BLIS build. Used for agg-4xtp4 and pd-thresh16.
#   blis-edpp — built externally. Used only for the pd-edpp arm.
#
# Output: $OUTDIR (default ./blis-pd-sweep-rate_threshold_vs_edpp)
...
#
# Usage:
#   examples/sweep-pd-rate_threshold_vs_edpp.sh
#   OUTDIR=./results examples/sweep-pd-rate_threshold_vs_edpp.sh
...
OUTDIR="${OUTDIR:-./blis-pd-sweep-rate_threshold_vs_edpp_2}"
CHAT_RATES="${CHAT_RATES:-10 30 50 80 100 120 150 200}"
CODE_RATES="${CODE_RATES:-0.01 0.02 0.05 0.1 0.2 0.5 1 2 5}"
...
ARMS=(
  ...
  "pd-edpp|./exp/blis-edpp|--num-instances 4 --prefill-instances 1 --decode-instances 3 --prefill-tp 4 --decode-tp 4 --hardware H100 --pd-transfer-bandwidth 10.3 --pd-decider edpp"
)
