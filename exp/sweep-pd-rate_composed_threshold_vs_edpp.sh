...
#
# Binaries:
#   blis      — standard BLIS build
#   blis-edpp  — built externally
#
# Output: $OUTDIR (default ./blis-pd-sweep-rate_composed_threshold_vs_edpp)
...
#
# Usage:
#   examples/sweep-pd-rate_composed_threshold_vs_edpp.sh
#   OUTDIR=./results examples/sweep-pd-rate_composed_threshold_vs_edpp.sh
#   CHAT_RATES="100 150" CODE_RATES="1 5" SEEDS="42" \
#     examples/sweep-pd-rate_composed_threshold_vs_edpp.sh
#   HORIZON_S=60 examples/sweep-pd-rate_composed_threshold_vs_edpp.sh

set -euo pipefail

cd "$(dirname "$0")/.."

OUTDIR="${OUTDIR:-./blis-pd-sweep-rate_composed_threshold_vs_edpp}"
...
ARMS=(
  ...
  "pd-edpp|./blis-edpp|--num-instances 4 --prefill-instances 1 --decode-instances 3 --prefill-tp 4 --decode-tp 4 --hardware H100 --pd-transfer-bandwidth 10.3 --pd-decider edpp"
)
